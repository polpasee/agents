...
export function FileAttentionPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const files = useMemo(() => {
    ...
    for (const agent of agents.values()) {
      for (const tc of agent.toolCalls) {
        ...
        const entry = map.get(filePath) || { path: filePath, reads: 0, edits: 0, total: 0 };
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
        {files.length === 0 && (
          <div className="text-xs text-center py-8" style={{ color: UI.text.empty }}>
        ...
        {files.map((file) => {
          ...
          return (
            <div key={file.path} className="rounded px-2 py-1.5" style={{ background: `${color}08`, border: `1px solid ${color}15` }}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono truncate" style={{ color, maxWidth: 200 }} title={file.path}>
                  {shortPath}
                </span>
              ...
              <div className="flex gap-2 mt-0.5 text-xs" style={{ color: UI.text.dimmed }}>
      ...
      {/* Summary */}
      {files.length > 0 && (
        <div className="px-3 py-1.5 text-xs font-mono" style={{ color: UI.text.dimmed, borderTop: "1px solid var(--color-border)" }}>
